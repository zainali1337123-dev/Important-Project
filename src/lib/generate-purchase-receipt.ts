...
export async function generatePurchaseReceiptPDF(params: {
  ...
  const PR_PRODUCT_COL_IDX = 1;
  const prProductName = product?.name ?? `Product #${purchase.product_id}`;
  const prProductCellImages = new Map<number, RenderedTextImage>();
  if (hasUrdu(prProductName)) {
    try {
      const image = renderMultilineTextToImageDataUrl(prProductName, {
        fontSize: 9.5,
        color: "#28323c",
        maxWidthPt: (50 - 5) * 2.835,
        lineHeightPt: 9.5 * 1.15,
        scale: 3,
      });
      prProductCellImages.set(0, image);
    } catch (e) {
      console.warn("Purchase-receipt Urdu cell image render failed:", e);
    }
  }
  ...
  autoTable(doc, {
    ...
    didParseCell: (hookData) => {
      if (
        ...
        hookData.column.index === PR_PRODUCT_COL_IDX &&
    ...
    didDrawCell: (hookData) => {
      if (
        ...
        const image = prProductCellImages.get(hookData.row.index)!;
        ...
        try {
          ...
          console.warn("Purchase-receipt Urdu image embed failed:", e);
        }
      }
    },
    columnStyles: {
